...
class MachineStock( db.Model ):
    ...
    def make( machine_id: int, product_id: int , quantity: int ) -> Result:
        ...
        if not isinstance(product_id, int):
            return Result.error(f"Invalid product ID type. Expected int, got={type(product_id).__name__}")
        ...
        if quantity <= 0:
            return Result.error( f"Invalid quantity. ( n <= 0 )")

        # Prevent duplicate entry
        if MachineStock.get( machine_id=machine_id, product_id=product_id, ):
            return Result.error( f"An existing entry already exists for machine { machine_id } and product { target_product.product_id }")

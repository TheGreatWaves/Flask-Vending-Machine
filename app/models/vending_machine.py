...
class Machine( db.Model ):
    ...
    def edit( self, 
        ...
        if new_stock:
            ...
            delete_stock = MachineStock.__table__.delete().where( MachineStock.machine_id == self.machine_id )
            db.session.execute( delete_stock )

            # Add all new stock
            for product_id, quantity in new_stock:
                stock_info, message = self.add_product( product_identifier=product_id, quantity=quantity )

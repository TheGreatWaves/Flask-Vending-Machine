...
def add_product_to_machine(machine_id: int, product_identifier: (int | str)):
    ...
    if target_machine := Machine.find_by_id(machine_id):
        stock, message = target_machine.add_product(
            product_identifier=product_identifier, quantity=10
        )
...
def edit_machine(id):
    ...
    if machine := Machine.find_by_id(id):
        ...
        if content := request.get_json():
            ...
            def get_if_found(
                name): return None if name not in content else content[name]

            name = get_if_found("name")
            location = get_if_found("location")
            stock_list = get_if_found(
                "stock_list"
            ...
            val = machine.edit(
                new_name=name, new_location=location, new_stock=stock_information_list
            )

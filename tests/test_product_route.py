...
def test_product_creation(app, client, product_name, product_price):
    ...
    # product_price = 100.0
    response = client.post("/product/create", json={"product_name": product_name, "product_price": product_price})
    assert not Log.make_from_response(response=response).has_error()

    log: Log = Log.make_from_response(response=response)
    assert log.has_entry(broad="Product", specific=f"New Product: {product_name}, {product_price}")

    with app.app_context():
        product: Product = db.session.get(Product, {"product_id": 1})
        assert product is not None
        assert product.product_name == product_name and float(product.product_price) == float(product_price)
...
def test_product_creation_error(app, client):
    product_name = "sample_name"
    product_price = 100.0
    _ = client.post("/product/create", json={"product_name": product_name, "product_price": product_price})
    fail_response = client.post("/product/create", json={"product_name": product_name, "product_price": product_price})

    assert Log.make_from_response(response=fail_response).has_error(Product.ERROR_CREATE_FAIL)
...
def test_search_product_1(app, client):
    ...
    assert response_json.get("product_id") == 1 \
...
def test_search_product_2(app, client):
    _ = client.post("/product/create", json={"product_name": "product_1", "product_price": 100.00})
    _ = client.post("/product/create", json={"product_name": "product_2", "product_price": 100.00})

    search_response = client.get(f"product/search/product")
    assert len(search_response.json) == 2

    _ = client.post("/product/create", json={"product_name": "product_3", "product_price": 100.00})
...
def test_edit_product(app, client):
    ...
    create_response = client.post("/product/create",
    ...
    edit_response = client.post("/product/1/edit", json={"product_name": new_name, "product_price": new_price})

    edit_log: Log = Log.make_from_response(response=edit_response)

    assert not edit_log.has_error()
    assert edit_log.has_entry(broad="Product", specific=f"Product ID 1")
...
def test_edit_product_fail(app, client):
    # Product not found
    edit_response = client.post("/product/1/edit",
                                json={})
    assert Log.make_from_response(response=edit_response).has_error(Product.ERROR_NOT_FOUND)
    ...
    edit_response = client.post("/product/1/edit",
                                json={"product_name": original_name, "product_price": original_price})
    assert Log.make_from_response(response=edit_response).has_error(Product.ERROR_EDIT_FAIL)
...
def test_get_all_products_fail(app, client):
    get_all_response = client.get("/product/all")
    assert Log.make_from_response(response=get_all_response).has_error(Product.ERROR_NOT_FOUND)
...
def test_get_machine_with_product(app, client):
    ...
    with app.app_context():
        for machine_id in range(10):
            create_machine_response = client.post(f"machine/create/location_{machine_id}/John")
            assert not Log.make_from_response(response=create_machine_response).has_error()

            stock, _ = MachineStock.make(product_id=product_id, machine_id=machine_id + 1, quantity=10)
    ...
    for machine_id in range(10):
        ...
        assert machine.get("location") == f"location_{machine_id}" and machine.get("machine_name") == "John"


def test_get_machine_with_product_fail(app, client):
    # product not found
    not_found_response = client.get("/product/42/where")
    assert Log.make_from_response(response=not_found_response).has_error(Product.ERROR_NOT_FOUND)

    _ = client.post("/product/create",
                    json={"product_name": "product_1", "product_price": 100.0})

    # product not found in any machine
    no_machine_response = client.get("/product/1/where")
    assert Log.make_from_response(response=no_machine_response).has_error(Product.ERROR_NOT_FOUND)

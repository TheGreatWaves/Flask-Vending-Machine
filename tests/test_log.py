...
def test_copy_log():
    copy_from_log: Log = Log(log={"Weird words": Record(records={"Foo": ["Wow that is really weird."]})})

    assert copy_from_log.has_entry(
        broad="Weird words",
        specific="Foo"
    )
...
def test_entry_log():
    ...
    supernatural_discussions.add(
        specific="Aliens",
        new_record="I think they're real."
    )
    ...
    supernatural_discussions.add(
        specific="Aliens",
        new_record="I think you are onto something."
    )

    assert len(supernatural_discussions.records.get("Aliens")) == 2

    weird_discussions = log.entry("Forgotten Discussions")
    weird_discussions.add(
        specific="Why are we here?",
        new_record="Who knows. Foo."
    )
...
def test_iadd_log():
    log_1: Log = Log({"Weird Topics": Record({"Atlantis": ["I've seen atlantis, trust me!"]})})
    log_2: Log = Log({"Weird Topics": Record({"Aliens": ["What? It's just an air balloon."]})})
    log_3: Log = Log({"Weird Topics": Record({"Aliens": ["Not this again..."]})})
    log_4: Log = Log({"Weird People": Record({"John": ["Yeah that is really odd."]})})

    log_1 += log_2

    assert log_1.has_entry(
        broad="Weird Topics",
        specific="Aliens"
    )
    ...
    assert log_2.logs.get("Weird Topics").get("Aliens") == ["What? It's just an air balloon.", "Not this again..."]
